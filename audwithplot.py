...
def plot_waveform(audio_path, title):
    ...
    # Print key values
    print(f"Total samples in {title}: {len(samples)}")
...
class AudioSteganographyApp:
    def __init__(self, root):
        ...
        self.plot_cover_audio_button = tk.Button(self.encode_frame, text="Plot Cover Audio",
                                                 command=self.plot_cover_audio)
        self.plot_cover_audio_button.grid(row=5, column=0, padx=10, pady=10)

        self.plot_secret_audio_button = tk.Button(self.encode_frame, text="Plot Secret Audio",
                                                  command=self.plot_secret_audio)
        self.plot_secret_audio_button.grid(row=5, column=1, padx=10, pady=10)

        self.plot_encoded_audio_button = tk.Button(self.encode_frame, text="Plot Encoded Audio",
                                                   command=self.plot_encoded_audio)
        self.plot_encoded_audio_button.grid(row=5, column=2, padx=10, pady=10)
        ...
        self.plot_decoded_audio_button = tk.Button(self.decode_frame, text="Plot Decoded Audio",
                                                   command=self.plot_decoded_audio)
        self.plot_decoded_audio_button.grid(row=4, column=1, padx=10, pady=10)
    ...
    def plot_cover_audio(self):
        audio_path = self.cover_audio_path.get()
        if audio_path:
            plot_waveform(audio_path, "Cover Audio Waveform")
        else:
            messagebox.showerror("Error", "No cover audio file specified.")

    def plot_secret_audio(self):
        audio_path = self.secret_audio_path.get()
        if audio_path:
            plot_waveform(audio_path, "Secret Audio Waveform")
        else:
            messagebox.showerror("Error", "No secret audio file specified.")

    def plot_encoded_audio(self):
        audio_path = self.output_path.get()
        if audio_path:
            plot_waveform(audio_path, "Encoded Audio Waveform")
        else:
            messagebox.showerror("Error", "No encoded audio file specified.")

    def plot_decoded_audio(self):
        audio_path = self.decoded_output_path.get()
        if audio_path:
            plot_waveform(audio_path, "Decoded Audio Waveform")
        else:
            messagebox.showerror("Error", "No decoded audio file specified.")
