...
def encode_audio(cover_audio_path, secret_audio_path, output_path):
    ...
    # Encode secret audio into the cover audio
    encoded_samples = (cover_samples & 0xFFFE) | (secret_samples & 0x0001)

    encoded_audio = cover_audio._spawn(encoded_samples.astype(np.int16).tobytes())
    encoded_audio.export(output_path, format="wav")


def decode_audio(encoded_audio_path, output_path):
    ...
    decoded_samples = (encoded_samples & 0x0001) * (2 ** 15)
